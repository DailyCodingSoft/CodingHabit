import { StreakComponentProps } from "@/types";
import Image from "next/image";
import PlusButton from "@/components/ui/plus-button/PlusButton";

export default function Streak(props: StreakComponentProps) {
    const debt = props.debt ?? 0;
    const streak = props.streak ?? 0;

    return (
        <div className="relative w-72">
        <div className="bg-[var(--primary-light-color)] border-2 border-[var(--color-black)] rounded-lg text-center w-72 p-4 flex flex-col items-center">
            <Image src={props.user.image || "/place_holders/userpicture.png"} alt="avatar" className="rounded-full m-5" width={100} height={100}/>
            <p>{props.user.username}</p>
            <div className="flex gap-4 mt-2">
                <p>Deuda: {debt}</p>
                <p>Racha: {streak}</p>
            </div>
        </div>
        <div className="absolute top-2 right-2">
            <PlusButton onClick={props.onUpdateClick || (() => {})} isFixed={false} />
        </div>
        </div>
    )
}